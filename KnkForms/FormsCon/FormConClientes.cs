...
{
    public partial class FormConClientes : KnkForms.FormsCon.FormConPai
    {
        ...
        string query = "SELECT IdFornCliente, RazaoSocial, NomeFantasia, InscricaoEstadual, CpfCnpj, Tipo, IdCidade, IdRegiao, Logradouro, Numero, Complemento, Bairro, Cep, ConsumidorRevenda, Observacao, RegimeSemSt, ProdutorRural, DataUltimaCompra, Ativo, FisicaJuridica, IdEmpresa, DataCadastro, DataModificacao FROM FornCliente";

        public FormConClientes()
        {
            InitializeComponent();
            CarregaLV();
        }
                ...
                try
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                if (reader["Tipo"].ToString() == "C" || reader["Tipo"].ToString() == "A")
                                {
                                    ...
                                    item.SubItems.Add(reader["RegimeSemSt"].ToString());
                                    item.SubItems.Add(reader["ProdutorRural"].ToString());
                                    item.SubItems.Add(reader["DataUltimaCompra"].ToString());
        ...
        protected override void Alterar()
        {
            if (listVConsulta.SelectedItems.Count > 0)
            {
                ...
                oFormCadCliente.ConhecaObj(oCliente);
                oFormCadCliente.LimpaTxt();
                oFormCadCliente.CarregaTxt(campo1, campo2, campo3, campo4, campo5, campo6, campo7, campo8, campo9, campo10, campo11, campo12, campo13, campo14, campo15, campo16, campo17, campo18, campo19, campo20, campo21, campo22, campo23, campo24, campo25);
                oFormCadCliente.ShowDialog();
            }
            CarregaLV();
        }
        ...
        protected override void Pesquisar()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    MessageBox.Show("Erro ao pesquisar os dados de Clientes: " + ex.Message);
                }
            }
        }
        private void listV_DoubleClick(object sender, EventArgs e)
        {
            if (listVConsulta.SelectedItems.Count > 0)
            {
                ...
                if (cadContato != null)
                {
                    cadContato.SetClientes(idCliente, nomeCliente);
                }
